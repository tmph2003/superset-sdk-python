...
```bash
% superset-cli  --username admin --password admin https://superset-dev.sunhouse.com.vn/ sync dbt-core --merge-metadata --select models/gold/ --project=sunhouse_etl_pipeline --profile=sunhouse_etl_pipeline --target=dev --profiles=profiles.yml --max-workers=3 target/manifest.json
```
